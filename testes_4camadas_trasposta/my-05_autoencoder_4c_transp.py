...
# Decoder 1
#Wd_1 = tf.Variable(tf.truncated_normal([L2, L1], stddev=0.1))
Wd_1 = tf.transpose(We_2)  
bd_1 = tf.Variable(tf.truncated_normal([L1]))

#Decoder 2
#Wd_2 = tf.Variable(tf.truncated_normal([L1, Linp], stddev=0.1)) 
Wd_2 = tf.transpose(We_1)
bd_2 = tf.Variable(tf.truncated_normal([Linp]))
...
D1 = tf.nn.sigmoid(tf.nn.bias_add(tf.matmul(C2, Wd_1), bd_1))

# formula para as predicoes
X_ = tf.nn.sigmoid(tf.nn.bias_add(tf.matmul(D1, Wd_2), bd_2))
...
mnist = mnist_data.read_data_sets(DATA_DIR_FASHION, one_hot=True, reshape=False, validation_size=0)
###############################
...
mnist_ = mnist_data.read_data_sets(DATA_DIR_FASHION, one_hot=False, reshape=False, validation_size=0)
labels = mnist_.test.labels 
print ('\ndimensao da matriz de labels: '+ str(labels.shape))

#extrai codigos
codigos = sess.run(C2, feed_dict=testData)
...
plt.savefig('fashion_' + str(L2) + '_' + str(lossTestOk) + '.png')
plt.close()
